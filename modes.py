...
def DY_3fld(i, Y, A, K, wC, DwC, cs2C, wN, DwN, cs2N,
    ...
    DvC = -H[i]*(1-3*wC[i])*vC - vC*DwC[i]/(1+wC[i]) + K*deltaC*cs2C[i,:]/(1+wC[i]) + K*Phi
    
    DdeltaN =  -(1+wN[i])*(K*vN-3*DPhi) - 3*H[i]*(cs2N[i,:]-wN[i])*deltaN
    DvN = -H[i]*(1-3*wN[i])*vN - vN*DwN[i]/(1+wN[i]) + K*deltaN*cs2N[i,:]/(1+wN[i]) + K*Phi
...
def DY_2fld(i, Y, A, K, wD, DwD, cs2D, OmegaD,
    ...
    DvD = -H[i]*(1-3*wD[i])*vD - vD*DwD[i]/(1+wD[i]) + K*deltaD*cs2D[i,:]/(1+wD[i]) + K*Phi
    dY[0, :] = DPhi
    dY[1, :] = DdeltaG
    dY[2, :] = DvG
    dY[3, :] = DdeltaD
